...
def analyze_temporal_relationship_diff(region, quadrant=None):
    """Analyze temporal relationships between TP and (TP-dvdt) for a region"""
    # Set up the columns
    if quadrant:
    ...
    # Calculate TP-dvdt
    diff_norm = tp_smooth_norm - dvdt_norm
    
    # Find peaks and troughs
    tp_peaks, _ = find_peaks(tp_smooth_norm, distance=50)
    tp_troughs, _ = find_peaks(-tp_smooth_norm, distance=50)
    diff_peaks, _ = find_peaks(diff_norm, distance=50)
    diff_troughs, _ = find_peaks(-diff_norm, distance=50)
    ...
    ax1.plot(time_data, tp_smooth_norm, label='Total Pressure Force', color='blue', alpha=0.7)
    ax1.plot(time_data, diff_norm, label='TP-dv/dt', color='purple', alpha=0.7)
    ...
    ax1.scatter(time_data[tp_troughs], tp_smooth_norm[tp_troughs], color='blue', marker='v', 
                label='TP troughs')
    ax1.scatter(time_data[diff_peaks], diff_norm[diff_peaks], color='purple', marker='^', 
                label='TP-dv/dt peaks')
    ax1.scatter(time_data[diff_troughs], diff_norm[diff_troughs], color='purple', marker='v', 
                label='TP-dv/dt troughs')
    ...
    for cycle in cycles:
        ...
        diff_peaks_in_cycle = diff_peaks[(time_data[diff_peaks] >= time_data[cycle_start]) & 
                                       (time_data[diff_peaks] <= time_data[cycle_end])]
        
        if len(tp_peaks_in_cycle) > 0 and len(diff_peaks_in_cycle) > 0:
            # Calculate phase difference for each peak pair
            for tp_peak in tp_peaks_in_cycle:
                # Find closest difference peak
                closest_diff_peak = diff_peaks_in_cycle[np.argmin(np.abs(time_data[diff_peaks_in_cycle] - 
                                                                        time_data[tp_peak]))]
                time_diff = time_data[tp_peak] - time_data[closest_diff_peak]
    ...
    ax1.set_title(f'{region}{title_suffix}\nTemporal Analysis of Total Pressure Force and TP-dv/dt')
...
with PdfPages('temporal_analysis_diff.pdf') as pdf:
    # First create main region plots
    for region in regions:
        fig = analyze_temporal_relationship_diff(region)
        pdf.savefig(fig)
        plt.close(fig)
        
        # Then create quadrant plots
        for quadrant in quadrants:
            fig = analyze_temporal_relationship_diff(region, quadrant)
            pdf.savefig(fig)
            plt.close(fig)

print("Temporal analysis plots for TP vs (TP-dv/dt) have been saved to 'temporal_analysis_diff.pdf'") 